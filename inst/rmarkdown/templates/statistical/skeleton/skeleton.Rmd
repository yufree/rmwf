...
```{r}
library(limma)
design <- model.matrix(~factor(mzrt$group))
...
library(caret)
## Spliting data
trainIndex <- createDataPartition(fs$group, p = .5, 
                                  list = FALSE, 
                                  times = 1)
## Get the training and testing datasets
train <- fs$data[, trainIndex]
train <- cbind.data.frame(Y=fs$group[trainIndex],t(train))
test  <- fs$data[,-trainIndex]
test  <- cbind.data.frame(Y=fs$group[-trainIndex],t(test))
## Train the model
library(doParallel)
