...
tsub <- tsub %>%
    ...
    peakPick() %>%
    peakAlign() %>%
    # default 1%, keep ions appearred in at least 1% pixels
    peakFilter() %>%
    process()
...
sum(eigs[1:4]) / sum(eigs)

# extract PC by the number found in last step
library(Cardinal)
mse_peakref_pca <- PCA(one, ncomp=4)
# extract PC data
pc_scores <- DataFrame(resultData(mse_peakref_pca, 1, "scores"))
# subset for cluster number optimization
subdf <- pc_scores[sample(nrow(pc_scores),10000),]
library(ClusterR)
opt = Optimal_Clusters_KMeans(as.data.frame(subdf), max_clusters = 10, plot_clusters = T, criterion = 'silhouette',initializer = 'optimal_init', tol_optimal_init = 0.2)
...
dfms1 <- dfyoung[,ms%in%dfx3$ms1]
dfms2 <- dfyoung[,ms%in%dfx3$ms2]
dfmsyoung <- apply(dfms1, 1, sum)
dfmsyoung2 <- apply(dfms2, 1, sum)
dfms1 <- dfold[,ms%in%dfx3$ms1]
dfms2 <- dfold[,ms%in%dfx3$ms2]
dfmsold <- apply(dfms1, 1, sum)
dfmsold2 <- apply(dfms2, 1, sum)
xy$pmd14h <- c(dfmsold,dfmsyoung)
xy$pmd14l <- c(dfmsold2,dfmsyoung2)
xy$pmd14 <- xy$pmd14h+xy$pmd14l

dfms1 <- dfyoung[,ms%in%dfx4$ms1]
dfms2 <- dfyoung[,ms%in%dfx4$ms2]
dfmsyoung <- apply(dfms1, 1, sum)
dfmsyoung2 <- apply(dfms2, 1, sum)
dfms1 <- dfold[,ms%in%dfx4$ms1]
dfms2 <- dfold[,ms%in%dfx4$ms2]
dfmsold <- apply(dfms1, 1, sum)
dfmsold2 <- apply(dfms2, 1, sum)
xy$pmd16h <- c(dfmsold,dfmsyoung)
xy$pmd16l <- c(dfmsold2,dfmsyoung2)
xy$pmd16 <- xy$pmd16h+xy$pmd16l

dfms1 <- dfyoung[,ms%in%dfx5$ms1]
dfms2 <- dfyoung[,ms%in%dfx5$ms2]
dfmsyoung <- apply(dfms1, 1, sum)
dfmsyoung2 <- apply(dfms2, 1, sum)
dfms1 <- dfold[,ms%in%dfx5$ms1]
dfms2 <- dfold[,ms%in%dfx5$ms2]
dfmsold <- apply(dfms1, 1, sum)
dfmsold2 <- apply(dfms2, 1, sum)
xy$pmd2h <- c(dfmsold,dfmsyoung)
xy$pmd2l <- c(dfmsold2,dfmsyoung2)
xy$pmd2 <- xy$pmd2h+xy$pmd2l

dfms1 <- dfyoung[,ms%in%dfx6$ms1]
dfms2 <- dfyoung[,ms%in%dfx6$ms2]
dfmsyoung <- apply(dfms1, 1, sum)
dfmsyoung2 <- apply(dfms2, 1, sum)
dfms1 <- dfold[,ms%in%dfx6$ms1]
dfms2 <- dfold[,ms%in%dfx6$ms2]
dfmsold <- apply(dfms1, 1, sum)
dfmsold2 <- apply(dfms2, 1, sum)
xy$pmd18h <- c(dfmsold,dfmsyoung)
xy$pmd18l <- c(dfmsold2,dfmsyoung2)
xy$pmd18 <- xy$pmd18h+xy$pmd18l
# check reaction level changes
t.test(xy$pmd2~xy$group)
t.test(xy$pmd14~xy$group)
t.test(xy$pmd16~xy$group)
t.test(xy$pmd18~xy$group)
```
