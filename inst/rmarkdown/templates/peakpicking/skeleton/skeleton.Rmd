...
XCMS is popular in metabolomics data analysis. Before we use XCMS to extract the peaks, we need to optimize the parameters to get peaks. Tutorial is [here](https://bioconductor.org/packages/devel/bioc/vignettes/IPO/inst/doc/IPO.html).
...
Here we use venn diagram to compare the results from IPO and default setting. However, peaks number is only one method to compare and you might need to check EIC for a fair comparison and related code is in the Visualization workflow.

```{r}
# align the peaks list
re <- enviGCMS::getcompare(mzrt,mzrt3)
ipo <- paste0(round(re[[1]]$mz,4),'@',round(re[[1]]$rt,1))
default <- paste0(round(re[[3]]$mz,4),'@',round(re[[3]]$rt,1))
compare <- fromList(list(ipo,default))
library(UpSetR)
compare <- fromList(list(ipo=ipo,default=default))
upset(compare, order.by = "freq",nsets = 2)
