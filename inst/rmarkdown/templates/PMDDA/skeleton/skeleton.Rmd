...
download.file(url, name)
path <- getwd()
ddafilem <- list.files(path,pattern = '*.mzML',recursive = T,full.names = T)
setwd('..')
...
setwd('..')
# DDA negative data
dir.create(file.path('DDAn'), showWarnings = FALSE)
setwd(file.path('DDAn'))
article_id <- 13252655
details <- fs_details(article_id)
name <- sapply(details$files,function(x) x$name)
url <- sapply(details$files,function(x) x$download_url)
download.file(url, name)
path <- getwd()
ddafilen <- list.files(path,pattern = '*.mzML',recursive = T,full.names = T)
setwd('..')

# matrix DDA negative data
dir.create(file.path('DDAMatrixn'), showWarnings = FALSE)
setwd(file.path('DDAMatrixn'))
article_id <- 13252661
details <- fs_details(article_id)
name <- sapply(details$files,function(x) x$name)
url <- sapply(details$files,function(x) x$download_url)
download.file(url, name)
path <- getwd()
ddafilemn <- list.files(path,pattern = '*.mzML',recursive = T,full.names = T)
setwd('..')
...
download.file(url, name)
path <- getwd()
pmddafilen <- list.files(path,pattern = '*.mzXML',recursive = T,full.names = T)
setwd('..')

# compare with CAMERA/RAMClustR
# CAMERA positive data
...
download.file(url, name)
path <- getwd()
ramclustrfile <- list.files(path,pattern = '*.mzXML',recursive = T,full.names = T)
setwd('..')

# CAMERA negative data
...
download.file(url, name)
path <- getwd()
camerafilen <- list.files(path,pattern = '*.mzXML',recursive = T,full.names = T)
setwd('..')
# RAMClustR negative data
...
download.file(url, name)
path <- getwd()
ramclustrfilen <- list.files(path,pattern = '*.mzXML',recursive = T,full.names = T)
setwd('..')
```
...
# new list, update group and remove pool qc/blk and save the new csv file
qcindex <- grepl('matrix',mzrt$group$sample_group)
mzrtfiltern <- enviGCMS::getfilter(mzrt,rowindex = indexn,colindex = !qcindex)
sum(index <- indexmean&indexrsd)
# 3608
...
posgnps <- read.csv('GNPS/posnodeanno.csv')
neggnps <- read.csv('GNPS/negnodeanno.csv')

library(eulerr)
num <- as.numeric(table(posgnps$DefaultGroups))
# 137
names(num) <- c('DDA','DDA&Matrix','DDA&Matrix&PMDDA&CAMERA','DDA&Matrix&PMDDA&CAMERA&RAMClustR','DDA&Matrix&CAMERA','DDA&Matrix&CAMERA&RAMClustR','DDA&Matrix&RAMClustR','DDA&PMDDA','DDA&PMDDA&CAMERA','DDA&PMDDA&CAMERA&RAMClustR','DDA&PMDDA&RAMClustR','DDA&CAMERA','DDA&CAMERA&RAMClustR','DDA&RAMClustR','Matrix','Matrix&CAMERA&RAMClustR','PMDDA','PMDDA&CAMERA','PMDDA&CAMERA&RAMClustR','PMDDA&RAMClustR','CAMERA','CAMERA&RAMClustR','RAMClustR')

sum(num[grepl('DDA',names(num))])
# 104
sum(num[grepl('Matrix',names(num))])
# 19
sum(num[grepl('PMDDA',names(num))])
# 99
sum(num[grepl('CAMERA',names(num))])
# 66
sum(num[grepl('RAMClustR',names(num))])
# 81
# 118 unique
fit1 <- euler(num)
png('comparepos.png',res=300,width = 2000,height = 2000)
plot(fit1,quantities = TRUE,fills = list(fill = c("red", 'green','yellow',"steelblue4"), alpha = 0.5))
dev.off()

numn <- as.numeric(table(neggnps$DefaultGroups))
# 73
names(numn) <- c('Matrix','DDA&Matrix','DDA&Matrix&RAMClustR','DDA&Matrix&PMDDA&CAMERA&RAMClustR','DDA&Matrix&CAMERA&RAMClustR','DDA&Matrix&CAMERA','DDA','DDA&RAMClustR','DDA&PMDDA&RAMClustR','DDA&PMDDA&CAMERA&RAMClustR','DDA&PMDDA','DDA&CAMERA&PMDDA','DDA&CAMERA','RAMClustR','RAMClustR&PMDDA','PMDDA','PMDDA&CAMERA','CAMERA')
sum(numn[grepl('DDA',names(num))])
# 52
sum(numn[grepl('Matrix',names(num))])
# 18
sum(numn[grepl('PMDDA',names(num))])
# 41
sum(numn[grepl('CAMERA',names(num))])
# 25
sum(numn[grepl('RAMClustR',names(num))])
# 35
# 55 unique
fit2 <- euler(numn)
...
df1 <- posgnps[,c(10:14)]
df1[df1>0] <- 1
colnames(df1) <- c('DDA','Matrix','PMDDA','CAMERA','RAMClustR')
df2 <- neggnps[,c(13:18)]
df2[df2>0] <- 1
colnames(df2) <- c('Matrix','DDA','RAMClustR','PMDDA','CAMERA')
df1 <- cbind.data.frame(df1,group = 'positive')
df2 <- cbind.data.frame(df2[,c(2,1,4,5,3)],group = 'negative')
df <- rbind.data.frame(df1,df2)
df$group <- factor(df$group,levels = c('positive','negative'))
fit <- euler(df,by=list(group))
png('compare.png',res=300,width = 3000,height = 1500)
plot(fit,quantities = TRUE,labels=NULL,legend = T)
dev.off()
```
...
mz <- xcms::precursorMz(dda_spectra)
rt <- xcms::rtime(dda_spectra)

dda_data <- MSnbase::readMSData(ddafilem, mode = "onDisk")
cwp <- xcms::CentWaveParam(snthresh = 5, noise = 100, ppm = 10,
                     peakwidth = c(3, 50))
dda_data <- xcms::findChromPeaks(dda_data, param = cwp)
dda_spectra <- xcms::chromPeakSpectra(dda_data)

mzm <- xcms::precursorMz(dda_spectra)
rtm <- xcms::rtime(dda_spectra)

plot(rt,mz,pch=19,xlab = 'retention time(s)',ylab = 'm/z',main='DDA MS1',xlim = c(0,1200),ylim = c(0,1200))

plot(rtm,mzm,pch=19,xlab = 'retention time(s)',ylab = 'm/z',main='DDA matrix MS1',xlim = c(0,1200),ylim = c(0,1200))

dda_data <- MSnbase::readMSData(ddafilen, mode = "onDisk")
cwp <- xcms::CentWaveParam(snthresh = 5, noise = 100, ppm = 10,
                     peakwidth = c(3, 50))
dda_data <- xcms::findChromPeaks(dda_data, param = cwp)
dda_spectra <- xcms::chromPeakSpectra(dda_data)

mzn <- xcms::precursorMz(dda_spectra)
rtn <- xcms::rtime(dda_spectra)

dda_data <- MSnbase::readMSData(ddafilemn, mode = "onDisk")
cwp <- xcms::CentWaveParam(snthresh = 5, noise = 100, ppm = 10,
                     peakwidth = c(3, 50))
dda_data <- xcms::findChromPeaks(dda_data, param = cwp)
dda_spectra <- xcms::chromPeakSpectra(dda_data)

mzmn <- xcms::precursorMz(dda_spectra)
rtmn <- xcms::rtime(dda_spectra)

plot(rtn,mzn,pch=19,xlab = 'retention time(s)',ylab = 'm/z',main='DDA MS1',xlim = c(0,1200),ylim = c(0,1200))

plot(rtmn,mzmn,pch=19,xlab = 'retention time(s)',ylab = 'm/z',main='DDA matrix MS1',xlim = c(0,1200),ylim = c(0,1200))
...
pmdmz <- xcms::precursorMz(dda_spectra)
pmdrt <- xcms::rtime(dda_spectra)

plot(pmdrt,pmdmz,pch=19,xlab = 'retention time(s)',ylab = 'm/z',main='PMDDA MS1',xlim = c(0,1200),ylim = c(0,1200))

dda_data <- MSnbase::readMSData(pmddafilen, mode = "onDisk")
cwp <- xcms::CentWaveParam(snthresh = 5, noise = 100, ppm = 10,
                     peakwidth = c(3, 50))
dda_data <- xcms::findChromPeaks(dda_data, param = cwp)
dda_spectra <- xcms::chromPeakSpectra(dda_data)

pmdmzn <- xcms::precursorMz(dda_spectra)
pmdrtn <- xcms::rtime(dda_spectra)

plot(pmdrtn,pmdmzn,pch=19,xlab = 'retention time(s)',ylab = 'm/z',main='PMDDA MS1',xlim = c(0,1200),ylim = c(0,1200))
```
